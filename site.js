(() => {
  ...
  const screenshots = [
    ["1-settings.jpg", "Setup — folders, saved access, offline-warning and notification controls"],
    ["1.2-settings.jpg", "Setup — notifications, Automatic Library Index, saved index and diagnostics"],
    ...
    ["8.6-health-check.jpg", "Health Check — Album / Track-Disc options, missing-record and folder-relink actions"],
  ...
  const revision = "20260914b";
  const previewSource = file => `screenshots/thumbs/${file}?v=${revision}`;
  ...
  if (gallery) {
    ...
    screenshots.forEach((item, index) => {
      ...
      image.src = previewSource(item.file);
  ...
  document.querySelectorAll("[data-media-showcase]").forEach(showcase => {
    ...
    const activate = (tab, focusTab = false) => {
      ...
      previewImage.src = previewSource(file);
